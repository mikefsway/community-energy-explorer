...
const TYP = {
  ...
  latent:     { label: "Latent",    blurb: "good conditions, little community energy yet — easy wins" },
  pioneering: { label: "Pioneering", blurb: "community energy despite thin conditions" },
  cold:       { label: "Cold",      blurb: "thin conditions and little community energy" },
};
const IMD_Q = ["#cb4b3a", "#e0855a", "#f0bd87", "#cdd9c4", "#9fb59a"]; // q1 most deprived → q5

let DATA = null, LADS = [], MED = {};
...
async function init() {
  ...
  document.querySelectorAll(".lens-tab").forEach(b =>
    b.addEventListener("click", () => switchLens(b.dataset.lens, b)));
...
const COLS = [
  ...
  { k: "need_p", label: "Need", fmt: v => pct(v), bar: "need" },
  { k: "ready_p", label: "Ready", fmt: v => pct(v), bar: "ready" },
  { k: "presence_p", label: "Presence", fmt: v => pct(v), bar: "pres" },
...
function renderTable() {
  const maxkw = Math.max(...LADS.map(d => d.biggest_kw));
  document.getElementById("lens-panel").innerHTML = `
    <div class="controls">
      ...
      <select id="t-typ" class="ctrl-input">
        ...
        ${Object.entries(TYP).map(([k, v]) => `<option value="${k}">${v.label}</option>`).join("")}
      </select>
      <select id="t-quick" class="ctrl-input">
        <option value="all">All authorities</option>
        <option value="none">No community energy</option>
        <option value="redress_no_ce">Redress but no community energy</option>
        <option value="high_need">Most deprived third</option>
      </select>
      <label class="ctrl-slider">Min project size
        <input id="t-kw" type="range" min="0" max="${Math.ceil(maxkw)}" step="10" value="${tstate.minkw}">
        <span id="t-kw-val">${tstate.minkw ? tstate.minkw + " kW" : "any"}</span>
      </label>
    ...
    <div class="table-scroll"><table class="data-table"><thead><tr>
      ${COLS.map(c => `<th data-k="${c.k}" class="${c.align === "left" ? "tl" : ""} ${c.k === sortKey ? "sorted " + (sortDir > 0 ? "asc" : "desc") : ""}">${c.label}</th>`).join("")}
  ...
  const kwIn = document.getElementById("t-kw");
  kwIn.oninput = e => {
    tstate.minkw = +e.target.value;
    document.getElementById("t-kw-val").textContent = tstate.minkw ? tstate.minkw + " kW" : "any";
    fillTable();
  };
...
function downloadCsv() {
  const keys = ["code", "name", "pop", "need_p", "ready_p", "presence_p", "ce_orgs", "ce_sites",
    "cap_kw", "cap_sites", "biggest_kw", "redress_grantees", "redress_total", "know_km",
...
function renderTypology() {
  ...
  const xm = MED.ready * 100, ym = MED.pres * 100;
  const sx = v => PAD + v * (W - 2 * PAD);
  const sy = v => H - PAD - v * (H - 2 * PAD);
  const quads = [
    { t: "thriving", at: [0.80, 0.90] },    // high ready, high presence — top-right
    { t: "latent", at: [0.80, 0.10] },      // high ready, low presence  — bottom-right
    { t: "pioneering", at: [0.16, 0.90] },  // low ready, high presence  — top-left
    { t: "cold", at: [0.16, 0.10] },        // low ready, low presence   — bottom-left
  ];
  const dots = LADS.map(d => `<circle cx="${sx(d.ready_p).toFixed(1)}" cy="${sy(d.presence_p).toFixed(1)}"
     ...
     stroke="#20251f" stroke-width="0.4" data-code="${d.code}" class="dot"><title>${esc(d.name)}</title></circle>`).join("");
  const qbg = `
  ...
  const qlabels = quads.map(q => `<text x="${sx(q.at[0])}" y="${sy(q.at[1])}" class="qlab badge-${q.t}" text-anchor="middle">${TYP[q.t].label.toUpperCase()}</text>`).join("");

  document.getElementById("lens-panel").innerHTML = `
    <p class="lens-intro">Each authority placed by <strong>readiness</strong> (enabling conditions, →)
    against <strong>community-energy presence</strong> (↑). Dot colour is deprivation (red = most deprived),
    size is population. The bottom-right <span class="badge latent">Latent</span> quadrant — ready but
    empty — is where the sector could most easily grow. Click any dot or name.</p>
    <div class="typo-wrap">
      <svg viewBox="0 0 ${W} ${H}" class="scatter" id="scatter">
        ...
        ${qlabels}
        ${dots}
        <text x="${W / 2}" y="${H - 14}" text-anchor="middle" class="axlab">Readiness — enabling conditions →</text>
        <text x="16" y="${H / 2}" text-anchor="middle" class="axlab" transform="rotate(-90 16 ${H / 2})">Community-energy presence →</text>
      </svg>
  ...
  document.getElementById("typo-lists").innerHTML = lists.map(([t, title, f, s]) => {
    ...
    return `<div class="typo-list"><h3><span class="badge ${t}">${TYP[t].label}</span> ${title}</h3>
      <ol>${items.map(d => `<li data-code="${d.code}"><span>${esc(d.name)}</span>
        <em>${t === "latent" ? "opp " + (+d.opportunity).toFixed(2) : "need " + pct(d.need_p)}</em></li>`).join("")}</ol></div>`;
  }).join("");
...
function renderMismatch() {
  ...
  const cards = [
    ...
    },
    {
      ...
      note: d => `${d.know_km} km to nearest`,
    },
    {
    ...
    },
    {
      ...
      note: d => `need ${pct(d.need_p)} · IMD q${d.imd_q}`,
    },
  ];
  document.getElementById("lens-panel").innerHTML = `
...
function renderRedress() {
  ...
  const dots = withpc.map(d => `<circle cx="${sx(d.need_p).toFixed(1)}" cy="${sy(d.rpc).toFixed(1)}"
     ...
     <title>${esc(d.name)} — ${gbp(d.redress_total)}</title></circle>`).join("");
  ...
  document.getElementById("lens-panel").innerHTML = `
    ...
    <span class="rstat">Correlation of redress £/person with deprivation: <strong>r = ${r.toFixed(2)}</strong></span></p>
    <div class="typo-wrap">
      ...
      <div class="typo-side">
        ${listBlock("Deprived, but neither redress nor community energy", gap, d => "need " + pct(d.need_p))}
...
function openDrawer(code) {
  ...
  const axis = (k, v) => `<div class="d-axis"><span>${k}</span><span class="minibar wide"><i style="width:${pct(v)}%"></i></span><b>${pct(v)}</b></div>`;
  document.getElementById("drawer-body").innerHTML = `
    ...
    <div class="d-scores">
      <div class="d-score"><span>Opportunity</span><b>${(+d.opportunity).toFixed(2)}</b></div>
      <div class="d-score"><span>Struggle</span><b>${(+d.struggle).toFixed(2)}</b></div>
    </div>
    <div class="d-grid">
      ...
      ${row("Nearest knowledge base", d.know_km + " km")}
    ...
    <a class="d-maplink" href="index.html?lat=${d.lat}&lon=${d.lon}&z=9.2">View on the map →</a>`;
